...
- (2026-07-12 database exposure closure) Read-only firewall/container audit found CouchDB `5984` and unauthenticated Redis `6379` externally reachable: Docker published both on all interfaces and the Robot whitelist accepts `5432-6543`. Added and persisted a narrowly scoped `DOCKER-USER` drop for those two ports on public interface `enp0s31f6`. External CouchDB changed from HTTP 200 to 000 and Redis from `PONG` to closed; host-local dependencies and public Rizzoma health stayed green. No Robot firewall write occurred. Evidence: `screenshots/260712-1203-database-exposure-closure/`.
- (2026-07-12 production-service hardening in flight) On branch `codex/production-service-hardening`, added explicit production loopback binding, non-development session-secret enforcement with previous-secret rotation, Redis-backed session readiness, and graceful Socket.IO/Redis/Yjs shutdown. Added immutable blue/green systemd assets plus exact-SHA install/deploy helpers; the obsolete Docker-era deploy behavior is removed. Local gates passed: typecheck, 62 Vitest files / 292 passed / 3 skipped, and the 3,298-module production build. This is not yet a deployment claim: public nginx still targets Vite `:3100` → API `:8100` until merge, HTTPS canary, public Playwright acceptance, and atomic cutover complete. Worklog: [production service hardening](docs/worklog-260712-production-service-hardening.md).
