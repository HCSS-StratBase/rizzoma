...
- (2026-07-12 reality audit) Corrected the release label after inspecting the public Vite transform, active process environments, executable feature-flag logic, and the native component itself. The public site is the hardened **React/TipTap parity path**, not the native fractal renderer: `FEAT_RIZZOMA_PARITY_RENDER=1`, `FEAT_RIZZOMA_NATIVE_RENDER` unset, and `RizzomaTopicDetail` additionally requires `?render=native`; `NativeWaveView` remains read-only. The API runs with `NODE_ENV=production`, but the public frontend is source modules served by Vite in development mode. At 05:58 CEST the active API had 2,279 seconds of uptime and its current log contained 395 requests / 0 5xx, which is a short post-cutover sample rather than reliability evidence. The active and rollback lanes remain unsupervised and share CouchDB. The canonical `/mnt/c/Rizzoma` checkout is not the release checkout: it remains on `feature/native-fractal-port` at `6e988cc` with one tracked modification and 134 untracked entries; use the clean release checkout for release work until that user-owned tree is reconciled.
- (2026-07-12 production closeout) PR #57 source head `daa3f2f3` was already public-live, so the prior “not yet deployed” checkpoint was incorrect. Strict public acceptance exposed unread HTTP 500s and a false-green Follow-the-Green smoke. PR [#60](https://github.com/HCSS-StratBase/rizzoma/pull/60) removed proxy-sensitive HTTP self-fetching, hardened remote-blip and mobile navigation, and replaced the smoke with a real-control `2 → 1 → 0` contract. All CI gates passed, PR #60 merged as `fe6988fb`, and that exact tree is now public through nginx → Vite `:3100` → API `:8100` with RedisStore. Public health/OAuth passed; collaboration passed 10/10 at 39 ms; desktop/mobile Follow-the-Green passed `2 → 1 → 0`; the required desktop-width visual sweep passed. Evidence: `screenshots/260712-0530-pr60-production-final/`. The old `:3000`/`:8788` lane remains healthy for rollback; service supervision, physical iPhone Safari, and 500/1,000-blip sweeps remain open.
- (2026-07-12 post-merge) PR #57 merged the release code to `master` as `8840f552` from source `daa3f2f3`; PR #58 then landed the handoff and inspected evidence as `6db65e20`. Final-head CI 29175331401 and iOS 29175331404 passed: 62 files / 283 passed / 3 skipped, 3,298 build modules, collaboration 10/10 with 1 ms relay and zero remote REST PUTs, and enforced full-render perf at 120/120 with 101 lazy slots, 394.3 ms landing, 595.6 ms expanded, and 36 MB heap. Merged source is not yet deployed; production verification and topology hardening remain open.
...
- [x] Finish real upload pipeline: `src/server/routes/uploads.ts` validates MIME signatures, blocks executables, optionally streams through ClamAV, and persists local files behind wave-bound metadata and a revocable download router. Public/pre-signed S3 URLs are intentionally rejected until private object bytes can be proxied through the same ACL. The client helper remains cancelable and `RizzomaBlip` surfaces progress, preview, retry, and dismiss controls.
