...
- (2026-07-12 authenticated cursor identity candidate) Branch `codex/authenticated-cursor-identity` follows merged PR #64 on `master` at `2595d2de` and carries only the collaborator-identity implementation plus its documentation. Authenticated users now enter Yjs awareness synchronously across topic-root, nested-blip, and generic editor surfaces; names use the account name with email fallback, colors are deterministic from the stable user ID, reconnect re-announces awareness, and destroy broadcasts immediate removal. Measured gates passed: focused collaboration tests 27/27, full Vitest 64 files / 306 passed / 3 skipped, typecheck, and a 3,299-module production build. This is a draft code candidate, not browser acceptance or deployment: two real signed-in users must still verify names/colors/typing/reconnect/leave in Playwright. Worklog: [authenticated cursor identity](docs/worklog-260712-authenticated-cursor-identity.md).
