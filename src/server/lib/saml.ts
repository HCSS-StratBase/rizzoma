...
// Lazy instances are keyed by callback URL so proxy/LAN hosts cannot inherit
// the first request's ACS URL.
...
export function getSamlConfig(callbackUrl: string): SamlConfig {
  ...
  return {
    ...
    // Bind every assertion to a request ID generated by node-saml and reject
    // replayed/unsolicited responses. RelayState alone only correlates the
    // browser transaction; it does not validate SAML InResponseTo.
...
export function getSamlInstance(callbackUrl: string): SAML {
  let samlInstance = samlInstances.get(callbackUrl);
  if (!samlInstance) {
    const config = getSamlConfig(callbackUrl);
    samlInstance = new SAML(config);
    samlInstances.set(callbackUrl, samlInstance);
  }
  return samlInstance;
}
