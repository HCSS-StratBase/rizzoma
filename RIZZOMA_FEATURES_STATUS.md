...
- **Server safeguards** - `/api/uploads` inspects MIME signatures, blocks executables by signature/extension, optionally streams file buffers through ClamAV, requires edit access to the canonical blip, and stores opaque local-file metadata. `/uploads/:id` resolves current wave-read access on every request and sends private/no-store/nosniff responses. S3/MinIO fails closed until object streaming is ACL-proxied.
...
- **Tests** - `src/tests/routes.uploads.edgecases.test.ts` covers authentication, edit authorization, canonical-wave binding, wave mismatch, file/virus failures, metadata cleanup, S3 fail-closed behavior, private download, cache headers, and immediate known-URL revocation. `src/tests/client.editor.GadgetNodes.test.ts` exercises the chart/poll gadget parse/render/command helpers.
