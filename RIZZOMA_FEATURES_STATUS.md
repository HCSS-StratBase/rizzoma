...
- **Visual sweep hardening**: `npm run visual:sweep` and `npm run visual:coverage` now produce a broad fresh feature sweep plus a row-level coverage matrix for all 161 screenshot-valid rows. The current matrix has zero screenshot gaps and the final verdict is 98 green / 63 orange / 0 red; remaining orange rows are mostly non-screenshot runtime/backend/device checks.
