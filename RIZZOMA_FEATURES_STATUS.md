...
- **Visual sweep hardening**: `npm run visual:sweep` now captures a broad fresh feature sweep, but the manifest is grouped evidence rather than a one-PNG-per-row matrix; next pass should add row-level `covered_by` mapping for all 161 screenshot-valid rows and keep mobile deep-link loading under watch.
