...
## 🟢 Current Status
- **Active-only toolbar parity proof (2026-07-13, `feature/native-fractal-port`)**: after the user's latest Downloads screenshot showed every expanded blip displaying a full menu, `b517102b` fixed active-state handling so only the clicked active blip exposes `Edit / Collapse / Expand / link / gear`. `npm run build` passed; focused tests passed (3 files, 25 tests). Public proof [BLB proof 20260713T205010](https://138-201-62-161.nip.io/?layout=rizzoma#/topic/18fd97812660e69bf157d9dc5a00e553) includes a hard gate that exactly one visible `.blip-menu-container` exists after clicking the terminal blip, owned by that terminal `data-blip-id`; visual evidence lives in `screenshots/260713-225006-public-active-terminal-toolbar-proof/`.
- **Public BLB proof after SSO 502 repair (2026-07-13, `feature/native-fractal-port`)**: the bare public hostname `https://138-201-62-161.nip.io/` was cut over to the new app after Gmail SSO exposed a stale nginx upstream to dead `127.0.0.1:8101`. Public root and `/api/health` return 200; `/api/auth/google` returns 302 to Google with callback to the public hostname; `scripts/verify-blb-fractal-proof.mjs` passed against the public URL and created [BLB proof 20260713T203706](https://138-201-62-161.nip.io/?layout=rizzoma#/topic/18fd97812660e69bf157d9dc5a00740b). Evidence lives in `screenshots/260713-223655-public-blb-fractal-proof-after-sso-502-fix/`.
- **BLB dev proof (2026-07-13, `feature/native-fractal-port`)**: `scripts/verify-blb-fractal-proof.mjs` passed against the clickable dev URL and created [BLB proof 20260713T132301](https://dev.138-201-62-161.nip.io/?layout=rizzoma#/topic/18fd97812660e69bf157d9dc5a005c3a). Evidence lives in `screenshots/260713-152256-dev-https-blb-fractal-proof-clean/`. The PNGs visually verify topic-root bullets, collapsed root/nested/terminal rows with bullet + green `[+]`, expansion to bulleted bodies, and reload persistence. The only console error left in `result.json` is the expected initial unauthenticated `/api/auth/me` 401 before the proof user registers.
- **Post-BLB-fix verification (2026-07-13)**: `npm run build` passed; targeted route tests passed (4 files, 11 tests); full `npm run test` passed (55 files, 245 passed, 3 skipped). Direct `npx eslint`/`npm run lint -- --quiet` did not produce diagnostics but timed out in this WSL checkout, so it is not counted as a lint pass.
- **Physical-device closure (2026-04-24, Pixel 9 Pro XL / Chrome Android)**: `VF-108` is now green for `feature/rizzoma-core-features` after ADB wireless-device validation against the current branch through `adb reverse tcp:3000 tcp:3000`. Evidence lives in `screenshots/260424-2319-real-device-pixel9proxl-local/`: `003-local-branch-after-wait.png` / `006-cdp-android-auth.png` prove current-branch mobile auth, and `015-cdp-android-toolbar-compact-final.png` proves authenticated topic/blip layout after fixing the toolbar overlap found on the phone. The focused mobile test now includes a regression check for in-flow mobile blip menus. `screenshots/260424-025320-feature-sweep/BUILD_QUALITY_VERDICT.md` now rates 161 green / 0 orange / 0 red. Boundary: the first physical-phone capture against `https://138-201-62-161.nip.io` showed a stale public deployment without the latest X/Twitter auth button, so redeploy + public-phone smoke are still required before claiming the VPS is current; iPhone Safari remains untested.
- **Post-phone-fix verification (2026-04-24)**: `npm run typecheck`, `npm run lint:branch-context`, `git diff --check`, and `npm run test -- --run src/tests/client.BlipMenu.test.tsx src/tests/client.mobilePwa.test.tsx` passed (2 files, 26 tests). Full `npm run test` was attempted twice after the toolbar fix but was killed with exit `-1` after many suites had passed and without a final assertion summary; rerun in CI or a clean shell before citing a fresh full-suite pass for this patch.
- **Orange-row closure (2026-04-24, local CI/runtime)**: Added Redis 5 session-store coverage (`src/tests/server.session.test.ts`), Twitter/X OAuth2 PKCE coverage (`src/tests/server.authOauth.test.ts`), and focused mobile/PWA/offline coverage (`src/tests/client.mobilePwa.test.tsx`). Verification passed: `npm run typecheck` and full `npm run test` (48 files, 174 passed, 3 skipped, 0 failures). This moved the matrix to 160 green / 1 orange / 0 red before the later physical-device closure.
...
- **Production-target perf baseline (2026-04-24, public prod, full render, 100 blips)**: `RIZZOMA_BASE_URL=https://138-201-62-161.nip.io RIZZOMA_PERF_BLIPS=100 RIZZOMA_PERF_RENDER=full RIZZOMA_SNAPSHOT_DIR=screenshots/260424-0010-prod-perf-baseline npm run perf:harness` pass. Artifacts: `screenshots/260424-0010-prod-perf-baseline/metrics-1776982200094-*.json` and `render-1776982200094-*.png`. Landing-labels stage 1193.7ms, FCP 740ms, memory 33MB, labels 100/100. Expanded-root stage 524.5ms, FCP 740ms, memory 36MB, blips 101/100. `PERF_SNAPSHOT_DIR=screenshots/260424-0010-prod-perf-baseline PERF_BUDGET_EXPECTED_BLIPS=100 PERF_BUDGET_MIN_RATIO=1 node scripts/perf-budget.mjs` pass on stage-local budgets; optional `PERF_BUDGET_CHECK_TTF=1` still flags expanded-root absolute TTF at 3522.9ms.
...
- Visual residual from the 2026-04-24 full sweep: avatar fallbacks are now local initials/provider avatars with no broken external placeholders in the final public-prod screenshots. Runtime tests cover gestures, BottomSheet, touch targets/PWA/offline behavior, and physical Pixel 9 Pro XL / Chrome evidence closes the last matrix row for the active branch. Public VPS freshness and iPhone Safari remain separate validation risks.
...
- Mobile viewport validation is now CI-gated via `browser-smokes` job with `RIZZOMA_E2E_PROFILES=mobile`; check `follow-the-green-mobile/` snapshots for visual verification. Physical Android evidence is under `screenshots/260424-2319-real-device-pixel9proxl-local/`.
- Full visual sweeps are now runnable via `npm run visual:sweep`; use a new datetime-stamped `screenshots/<stamp>-feature-sweep/` folder for each full refresh, document residuals in the generated manifest, and run `npm run visual:coverage` to refresh row-level evidence.
