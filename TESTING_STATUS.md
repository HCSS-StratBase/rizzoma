...
## 🟢 Current Status
- **Visual feature sweep (2026-04-24, public prod)**: `RIZZOMA_BASE_URL=https://138-201-62-161.nip.io RIZZOMA_SWEEP_STAMP=260424-003739 npm run visual:sweep` pass. Artifacts: `screenshots/260424-003739-feature-sweep/` with 40 primary screenshots plus manifest, parsing 196 documented rows / 161 screenshot-valid rows / 69 dynamic candidates from `RIZZOMA_FEATURES_STATUS.md`. `RIZZOMA_SWEEP_DIR=screenshots/260424-003739-feature-sweep npm run visual:coverage` pass, producing `coverage.md`/`coverage.json`: 93 static screenshot-covered rows, 8 dynamic screenshot-covered rows, 58 non-screenshot/test-artifact rows, 2 explicit screenshot gaps, 0 needs-review rows. Supplemental dynamic run: `RIZZOMA_BASE_URL=https://138-201-62-161.nip.io RIZZOMA_E2E_PROFILES=desktop,mobile RIZZOMA_SNAPSHOT_DIR=screenshots/260424-003739-feature-sweep/follow-green npm run test:follow-green` pass, adding desktop/mobile follow-green screenshots and console logs in the same folder. See `docs/VISUAL_SCREENSHOT_SWEEP.md`.
...
- Visual residual from the 2026-04-24 coverage matrix: live cursors and typing indicators remain screenshot gaps until a two-client dynamic capture shows remote cursor/typing state.
