...
## Current Branch Notes (integration candidate, last refreshed 2026-07-12)
...
- The offline/auth candidate is isolated on `codex/offline-auth-isolation`,
  based on PR #65 source checkpoint `5a376119`. Production replay is disabled;
  REST/Yjs recovery state is owner-partitioned; cross-tab auth changes
  rebootstrap and rotate Socket.IO with empty packet buffers; and the shell
  becomes visibly read-only offline. Integrate PR
  #66's server socket authorization, authoritative sync user, and network-only
  transport policy with the client acknowledgement/owner checks and this
  branch's service-worker v2 legacy-cache purge before deployment.
