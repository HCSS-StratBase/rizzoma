...
- Re-read checkpoint: 2026-02-02 15:02 local — drift warnings below remain accurate; no new docs added/removed (note `docs/LINKS_REPARENT.md` is still missing).
...
codex exec '
  ...
  Priority focus:
  1) Perf/resilience sweeps for large waves, inline comments, playback, unread flows, and mobile; fix 1k-blip perf harness failures (TTF ~66–69s, memory 125MB) and the missing root label (1000/1001). Windowed render metrics are now captured; decide whether to skip more perf-mode writes and reduce large-wave TTF/memory.
...
- Perf harness (optional large-wave sanity): `npm run perf:harness` (seeds a 5k-blip wave and captures time-to-first-render metrics/screenshots under `snapshots/perf/`); log metrics/budgets when you run it.
  - Recent runs: 200-blip PASS (TTF 2173.8ms, FCP 260ms, 101/200 rendered); 1000-blip FAIL (2026-02-02) with `perfLimit=1001` and `x-rizzoma-perf=1` (TTF ~66–69s, FCP 432ms, memory 125MB, 1000/1000 rendered; windowed 200 ~6.7s landing, ~69s expanded). Check budgets via `scripts/perf-budget.mjs`.
