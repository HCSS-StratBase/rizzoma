...
- `docs/HANDOFF.md` now reflects `feature/rizzoma-core-features` as of 2026-04-25; refresh if more changes land.
- Screenshot artifacts use `screenshots/YYMMDD-HHMM[-SS]-purpose-label/`; read `screenshots/README.md` before adding new visual artifacts and keep screenshots, manifests, health JSON, and dumps inside the matching run folder.
- Re-read checkpoint: 2026-05-04 21:55 CEST — Hryhorii's pull-and-test report on `feature/rizzoma-core-features` surfaced 4 issues; all fixed + verified end-to-end on the live VPS dev container at `https://dev.138-201-62-161.nip.io`: bullet hierarchy survives save (#45, `cd9e626e`), `docker compose up` works (#46, `cd9e626e`), inline `[+]` opens from edit mode (#47, `f0d7658e` + `707a24f6`), OAuth callback URL no longer leaks `localhost` (#48, `02a57468`). New nginx vhost + LE cert at `dev.138-201-62-161.nip.io` → `:8200` for OAuth-friendly dev access. All 4 GH issues filed + closed. Worklog at `docs/worklog-260504.md`. Visual proof of #47 at `screenshots/issue-47-fix-verified.png`.
