...
Last refreshed: 2026-07-13 (`fix/blb-ensure-inline-expand`; PR #72 exact
...
portal moved from topic edit to view mode. PR #75 merged as exact `cb209dbd`,
but private green still failed because topic-editor remount lost local inline
expansion state. The current follow-up uses idempotent ensure-expanded creation
and is green at 113 files / 686 passed / 3 skipped plus a 3,319-module build.
Blue is stopped for writer isolation and nginx has not changed. Remaining gates are
...
Last refreshed (prior): 2026-04-23 late-night (`master` @ `60d84a67`, BLB doc ecosystem hardened + Hetzner blip on rizzoma.com fully fractally BLB-correct to depth 3. New [`docs/RIZZOMA_LEGACY_EDITOR_PLAYWRIGHT.md`](docs/RIZZOMA_LEGACY_EDITOR_PLAYWRIGHT.md) (seven rules for scripting the legacy editor, with verified selectors + reference flow); new §19 Pre-Commit BLB Checklist in [`docs/BLB_LOGIC_AND_PHILOSOPHY.md`](docs/BLB_LOGIC_AND_PHILOSOPHY.md); new "Hyperlink convention" rule (use proper markdown links + GDrive cloud URLs, never `/mnt/g`) added to SYSTEM_INSTRUCTIONS.md and synced to all 3 CLI configs. Hetzner subtree on the live [HTU topic](https://rizzoma.com/topic/d328493e8943bf079795b999e22b4f6e/0_b_cc4d_cihsa/) verified end-to-end: every `[+]` marker has `class="blip-thread folded"`. 3 commits (`7cfff90b`, `cb3dd4bc`, `60d84a67`); bundle on GDrive; 4 Tana entries today.)

Last refreshed (prior): 2026-04-22 late-night (`master` @ `87c5e988`, **VPS production-build path now green**. `app-prod` healthy on `:8201` alongside dev `:8200` (`curl /api/health → 200`). Three Dockerfile bugs fixed in sequence: CMD path (`aa63d4c5`), missing parse5 runtime dep (`b0ed1a15`), /app/logs + /app/data/uploads write permission (`b6f8793d`, `40cfb0e2`). All OAuth creds + new SESSION_SECRET wired to both services. CI gates tightened (#147, `87c5e988`): typecheck + vitest are now hard gates, install no longer continue-on-error. tsc 0 errors, vitest 186/193 (was 183). HTTPS still blocked on Hetzner Cloud firewall (port 80 inbound). See `docs/worklog-260422.md` "Late-night session".)

Last refreshed (prior): 2026-04-22 02:30am (`master` @ `37241169`, deployed to VPS. Today's fixes UI-verified empirically: gadget palette scoping (CouchDB shows YouTube only in active blip), Vite /uploads proxy (green PNG renders inline). Pre-existing test bug fixed (184/193 vitest). Pre-existing 37 TS errors cleaned (`tsc --noEmit` now 0). Playwright follow-the-green smoke PASSED against VPS. GitHub issues all closed. Project root cleaned 8.4GB. See `docs/worklog-260422.md`.)
...
Last refreshed (prior): 2026-04-22 evening (`master` @ `48d5608a`, comprehensive depth-feature audit complete. Verified every rich-editor feature at DEPTH-3 great-grandchild blips via Playwright on the live VPS: @mention popup, #tag, ~task, code block with 30-language selector, gadget palette (YouTube/Code/Poll/etc), image upload to disk. All work. **No depth-specific gating in any code path.** 4 new commits since `c4844c73` are test-only (screenshots + READMEs); VPS doesn't need re-deploy. See `docs/worklog-260422.md` for full matrix. Side cleanups identified: gadget palette routing too greedy, Vite `/uploads/*` proxy missing, topic-root toolbar focus-leak.)

Last refreshed (prior): 2026-04-22 morning (`master` @ `c4844c73`, BUG #43 fix LIVE on VPS. `linksRouter` mount at `/api` used to shadow `DELETE /api/blips/:id` via its catch-all `DELETE /:from/:to` — every blip-delete 404'd. Fix: remount linksRouter at `/api/links`, move `GET /:id/links` into blipsRouter. Same commit shipped `FEAT_ALL: "1"` in docker-compose (VPS dev server was tree-shaking features) + sphinx behind `profiles: ["search"]` (closes #42). Pulled + rebuilt on VPS 2026-04-21 23:53 UTC; verified `DELETE /api/blips/:id → {deleted:true}`. VPS env docs refreshed in `docs/VPS_DEPLOYMENT.md`. Bug writeup at `docs/BUG_DELETE_BLIP_SHADOW.md`. Issues #42 + #43 both closed.)
...
- Active branch: `fix/blb-ensure-inline-expand` (2026-07-13), based on merged PR
  #75 exact `cb209dbd`; nginx remains on stopped PR #72 blue while PR #75 is
  private on green but rejected by acceptance.
...
codex exec '
  ...
  Step 0: 
    ...
    - Continue in the isolated `/home/stephan/rizzoma-final-release` worktree on `fix/blb-ensure-inline-expand`; never modify the dirty canonical `/mnt/c/Rizzoma` checkout.
