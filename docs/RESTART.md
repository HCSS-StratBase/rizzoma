...
- Active branch (this session arc): `feature/native-fractal-port` (sibling of `feature/rizzoma-core-features`). Always cite branch + date when sharing status.
- `docs/HANDOFF.md` reflects `feature/native-fractal-port` as of 2026-07-13. Earlier states preserved as historical sections.
- Screenshot artifacts use `screenshots/YYMMDD-HHMM[-SS]-purpose-label/`; read `screenshots/README.md` before adding new visual artifacts.
- Latest checkpoint: 2026-07-14 — local repo, pushed branch, and VPS checkout all include the `ac6a6f9d` parity-gate evidence checkpoint plus subsequent status-doc sync commits. Public root and `/api/health` return 200. `npm run parity:gate` is the current blocking UI/parity gate: it requires the legacy reference screenshots, fresh public sweep, current coverage, saved side-by-side legacy/current PNGs, and written `PARITY_AUDIT.md`. Latest evidence: `screenshots/260713-225614-public-parity-sweep-feature-sweep/` plus `legacy-current-comparisons/PARITY_AUDIT.md`. Audit verdict is **FAIL / IN_PROGRESS**, not parity-complete: 200 documented rows, 159 classified rows, 24 old PNGs, 44 new PNGs, 104/159 visual-covered rows, 2 screenshot gaps, 10 comparison sheets. Boundary: inspect/classify all sheets, close realtime gaps, fix visible divergences, then rerun the gate before claiming fully functional.
- Re-read checkpoint: 2026-05-12 — Coverage lift day. Sweep stable at **45/45 PASS · 0 FAIL · 0 no-gate** through 6 lift cycles. PM matrix: 25 → **204 verified PASS** (×8.2), coverage % visual 31% → **97%**. CouchDB participants index (`bd8cea23`) closed Bug A's last bottleneck — 271 ms → 61 ms; total Ctrl+Enter 432 → 235 ms in profile (cumulative 1434 → 235, **6.1× faster**). Autosave fix cherry-picked to `feature/rizzoma-core-features` (`899f9196`). Worklog: `docs/worklog-260512.md`.
- Previous checkpoint: 2026-05-11 — Bug C / Task #190 RESOLVED at root via `isEditingRef` guard against TipTap autosave overwriting saved content with `<p></p>` on inline-child mount (`65e2a11c`). Sweep gates 43/44 → 44/44 PASS · 0 FAIL · 0 no-gate. Worklog: `docs/worklog-260511.md`.
- Previous checkpoint: 2026-05-10 — 17-commit Native fractal-port arc shipped on `feature/native-fractal-port`. Bug A (Ctrl+Enter latency 1434ms→430ms, partial), Bug B (nested Ctrl+Enter at depth 2+ FIXED), Bug C (NEW, identified — nested inline-marker rendering blocks deep-fractal coverage; Task #188). PM dashboard 5× redesigned with tabs (Live activity / Dev Phases / Feature Sweep), fractal accordion, N/A non-visual split, Jaccard best-match matcher. Visual sweep at 44/45 PASS. Live: `https://dev.138-201-62-161.nip.io/native-port-pm.html`. Worklog: `docs/worklog-260510.md`.
- Previous checkpoint: 2026-05-04 21:55 CEST — Hryhorii's pull-and-test report on `feature/rizzoma-core-features` surfaced 4 issues; all fixed + verified end-to-end on the live VPS dev container at `https://dev.138-201-62-161.nip.io`: bullet hierarchy survives save (#45, `cd9e626e`), `docker compose up` works (#46, `cd9e626e`), inline `[+]` opens from edit mode (#47, `f0d7658e` + `707a24f6`), OAuth callback URL no longer leaks `localhost` (#48, `02a57468`). New nginx vhost + LE cert at `dev.138-201-62-161.nip.io` → `:8200` for OAuth-friendly dev access. All 4 GH issues filed + closed. Worklog at `docs/worklog-260504.md`.
- Previous checkpoint: 2026-04-24 23:52 CEST — final public-prod visual/runtime verdict captured in `screenshots/260424-025320-feature-sweep/`; the current matrix has 42 screenshots, zero screenshot gaps, mobile topic content, two-client realtime cursor/typing evidence, focused auth/session/mobile/PWA/offline runtime tests, physical Pixel 9 Pro XL / Chrome evidence, and a 161 green / 0 orange / 0 red verdict. The phone pass exposed a mobile toolbar overlap that is fixed in `BlipMenu`; final local-device evidence is `screenshots/260424-2319-real-device-pixel9proxl-local/015-cdp-android-toolbar-compact-final.png`. Public VPS freshness is now corrected: app-code commit `69d6a8a9` is live at `https://138-201-62-161.nip.io`, the VPS source checkout is fast-forwarded to the latest docs/evidence checkpoint, public health is OK, and public Android Chrome cursor-inline proof is in `screenshots/260424-2350-real-device-pixel9proxl-public/`. Boundary: iPhone Safari remains untested.
...
codex exec '
  ...
  Priority focus:
  1) Visual/runtime verdict hardening: latest current public sweep is `screenshots/260713-225614-public-parity-sweep-feature-sweep/` (44 screenshots, 200 documented rows parsed / 159 classified rows, 104 visual-covered rows, 53 non-screenshot rows, 2 screenshot gaps). The legacy/current side-by-side sheets and audit live under `legacy-current-comparisons/`; the audit is **FAIL / IN_PROGRESS**. Run `npm run parity:gate` before any UI handoff. Keep the older `screenshots/260424-025320-feature-sweep/` as historical evidence for the previous `feature/rizzoma-core-features` sweep, not as the current native-fractal parity verdict. Keep iPhone Safari listed as a cross-browser risk until tested.
  2) Perf/resilience sweeps for large waves, inline comments, playback, unread flows, and mobile. `perf-harness.mjs` now supports `RIZZOMA_PERF_RENDER=lite|full`; public-prod full-render 100-blip baseline passed on 2026-04-24 (stage duration 1193.7ms landing / 524.5ms expanded, memory 33-36MB, artifacts in `screenshots/260424-0010-prod-perf-baseline/`). Next: rerun at 500/1000 blips, compare full vs lite, and investigate optional absolute-TTF drift.
  ...
  5) Keep health checks and CI gating for /api/health, inline comments, uploads wired (health-checks job runs npm run test:health); keep browser smokes green (toolbar-inline + follow-green desktop/mobile with FEAT_ALL=1).
  6) Automate bundles/backups (bundle + GDrive copy) and document cadence (`scripts/backup-bundle.sh`).
  7) Finish CoffeeScript/legacy cleanup and dependency upgrades; decide legacy static assets (note: `.coffee` files remain only in `original-rizzoma-src/` reference tree).
...
- Landing view parity: topic landing must match `screenshots/260224-2343-rizzoma-live-reference/feature/rizzoma-core-features/rizzoma-main.png` (root labels only, children/editor hidden until “+” is clicked). Keep perf harness landing metric on this collapsed state; measure expansions separately.
- April production work on `master` is not automatically on `feature/rizzoma-core-features`; verify branch-native content before assuming a `master` commit exists here.
...
- Perf harness (optional large-wave sanity): `npm run perf:harness` (seeds a 5k-blip wave and captures render metrics/screenshots under `snapshots/perf/`); set `RIZZOMA_PERF_RENDER=full` for production-like full rendering or leave default `lite` for the lean regression profile.
  - Recent runs: public-prod 100-blip full-render PASS (2026-04-24; stage 1193.7ms landing / 524.5ms expanded, memory 33-36MB, artifacts in `screenshots/260424-0010-prod-perf-baseline/`); 1000-blip PASS (2026-02-02) with `perfRender=lite` + `perfLimit=1000` + `x-rizzoma-perf=1` (stage duration ~1.5s landing / ~0.5s expanded, memory 23MB, 1000/1000 rendered; windowed 200 ~2.6–2.9s). Check budgets via `PERF_SNAPSHOT_DIR=<dir> node scripts/perf-budget.mjs`.
- Visual sweep: `npm run visual:sweep` captures the broad feature screenshot set and writes a manifest; `npm run visual:coverage` writes row-level `coverage.md`/`coverage.json`; latest public-prod folder is `screenshots/260424-025320-feature-sweep/`.
