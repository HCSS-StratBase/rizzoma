...
- Latest checkpoint: 2026-07-13 — BLB collapsed-row affordance fixed on `feature/native-fractal-port` (`75d888c7`) and verified on the dev VPS. Clickable proof: `https://dev.138-201-62-161.nip.io/?layout=rizzoma#/topic/18fd97812660e69bf157d9dc5a005c3a`; artifacts: `screenshots/260713-152256-dev-https-blb-fractal-proof-clean/`. Verification: `npm run build`, targeted route tests (11/11), full `npm run test` (55 files, 245 passed, 3 skipped), and visual PNG inspection. Dev nginx was repaired from dead `127.0.0.1:8101` to live `127.0.0.1:3000`. Boundary: dev proof only; production cutover and broad visual sweep remain pending.
...
codex exec '
  ...
  Priority focus:
  1) Visual/runtime verdict hardening: latest full sweep is `screenshots/260424-025320-feature-sweep/` (42 primary screenshots, 196 documented rows parsed / 161 screenshot-valid / 69 dynamic candidates, row-level coverage matrix with 101 static screenshot-covered / 2 dynamic screenshot-covered / 58 non-screenshot-test rows / 0 screenshot gaps; verdict 161 green / 0 orange / 0 red). Physical Pixel 9 Pro XL / Chrome evidence for VF-108 lives in `screenshots/260424-2319-real-device-pixel9proxl-local/`; `015-cdp-android-toolbar-compact-final.png` is the accepted post-fix toolbar screenshot. Public VPS is live from app-code commit `69d6a8a9`, source checkout is fast-forwarded to the latest docs/evidence checkpoint, and public Android Chrome cursor-inline proof is in `screenshots/260424-2350-real-device-pixel9proxl-public/`. Keep iPhone Safari listed as a cross-browser risk until tested.
