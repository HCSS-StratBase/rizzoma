...
- Re-read checkpoint: 2026-05-11 — Bug C / Task #190 RESOLVED at root via `isEditingRef` guard against TipTap autosave overwriting saved content with `<p></p>` on inline-child mount (`65e2a11c`). Sweep gates 43/44 → **44/44 PASS · 0 FAIL · 0 no-gate**. Autosave fix cherry-picked to `feature/rizzoma-core-features` as `899f9196`. Task #191 (Bug A last mile) investigated but no further latency win shipped — bottleneck is server-side participants fetch. Worklog: `docs/worklog-260511.md`.
