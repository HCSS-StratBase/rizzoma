...
Last refreshed: 2026-07-12 (`master` `9c4fb68f`; VPS checkout `3a55155a`; public runtime code `fe6988fb`). PR [#60](https://github.com/HCSS-StratBase/rizzoma/pull/60) is public-live and passed health/OAuth, collaboration 10/10 at 39 ms, strict real-control Follow-the-Green `2 → 1 → 0`, RedisStore, and the viewport sweep. Reality-audit correction merged through PR [#62](https://github.com/HCSS-StratBase/rizzoma/pull/62): public Vite runs in development mode with parity `1` and native unset, so this is the React/TipTap parity release, not a native-render cutover. API audit snapshot: 395 requests / 0 5xx after 2,279 seconds, which is not a soak. Both lanes remain unmanaged and share CouchDB. Tana release output: `8mGAbLRiBnne`. Evidence: `screenshots/260712-0530-pr60-production-final/`.)
