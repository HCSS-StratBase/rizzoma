...
3. **Playwright smokes against VPS**:
   - `test:toolbar-inline` — 16/17 toolbar selectors found (bold/italic/underline/strike/highlight/clear/bullet/ordered/links/attach/image/overflow/undo/redo). Failed on the post-edit transition selector (`.blip-menu-read-surface` timeout). **Root cause traced and fixed** (later in the day): commit `198656c5` (2026-04-14, "feat: topic auto-enters edit mode") made authenticated topic loads enter topic-root edit mode automatically. That mounts a second `.ProseMirror` for the topic editor. `editor = page.locator('.ProseMirror').first()` then matched the topic editor (rendered higher in the DOM) rather than the blip editor under test, so `keyboard.type(...)` went into the topic and the subsequent `[data-testid="blip-menu-bold"].first()` toggle did nothing useful. The Done click also targeted by `.first()` could collide with the topic toolbar's `Done` (different class but same visible label). Fix: scope every per-blip selector inside the edit/done flow to `[data-blip-id="${blipId}"] ...`. Selector `data-testid="blip-menu-read-surface"` itself is still present in `BlipMenu.tsx` line 638 — the smoke needed an updated lookup, not a code change.
