...
Last Updated: 2026-07-12 (`codex/sharing-access-control-stack`; rebased onto merged hardening commit `2595d2de`, tree-identical to source head `dda4d1d5`; **not merged or deployed**). Persisted private/link/public sharing, viewer/commenter/editor/owner enforcement, session-backed Socket.IO authorization, live demotion, and role-selecting invitations are layered onto the managed runtime. Production inventory measured **26 topic metadata documents: 0 explicit policies, 26 missing-policy legacy documents, and 0 malformed policies**; all existing topics therefore use the documented public-read-only outsider fallback while owners retain management, and new topics are private. Details: [sharing and authorization reference](SHARING_AUTHORIZATION.md).
...
- Active development branch: `codex/sharing-access-control-stack` (2026-07-12; rebased onto merged hardening commit `2595d2de`, whose tree matches source head `dda4d1d5`; undeployed). Public production remains at application commit `fe6988fb` (PR #60). Always include branch name + date when summarizing status.
...
Current State (`codex/sharing-access-control-stack` on merged hardening `master` @ 2026-07-12; public production still `fe6988fb`, PR #60)
- The sharing/access candidate is based on merged hardening commit `2595d2de` and remains unmerged and undeployed. Production inventory measured 26 topic metadata documents: 0 explicit policies, 26 missing-policy legacy documents, and 0 malformed policies. Staging/public gates remain the full anonymous/outsider/viewer/commenter/editor/owner matrix plus live Socket.IO demotion.
