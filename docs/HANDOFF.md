...
- Integration dependency: the upload route and collaboration provider require PR #66's centralized sharing resolver, authenticated Socket.IO session, per-blip access, revocation, awareness validation, logout disconnect, and authoritative `user.id` sync response. This preintegration branch intentionally does not invent those APIs and must not deploy standalone.
