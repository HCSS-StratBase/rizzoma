...
Last Updated: 2026-07-12 (follow-on candidate `codex/authenticated-cursor-identity`; `master` base `2595d2de`, merged PR #64; public runtime code `fe6988fb`). Authenticated account names and deterministic colors now feed Yjs cursor awareness across topic-root, nested-blip, and generic editor surfaces; reconnect and leave behavior are explicit. Measured local gates passed: focused collaboration tests **27/27**, full Vitest **64 files / 306 passed / 3 skipped**, typecheck, and a **3,299-module** production build. This is not browser acceptance or deployment: the draft remains blocked on Playwright with two real signed-in users. See the [authenticated cursor identity worklog](worklog-260712-authenticated-cursor-identity.md).
