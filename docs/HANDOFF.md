...
- Visual residual from 2026-04-24 public-prod screenshots: mobile gesture, BottomSheet, touch-target/PWA, and offline behavior now have focused runtime tests; physical Pixel 9 Pro XL / Chrome closes the last branch-matrix row. Public VPS freshness was corrected by deploying app-code commit `69d6a8a9`; public Android Chrome cursor-inline proof lives in `screenshots/260424-real-device-pixel9proxl-public/`. VPS source is fast-forwarded to the latest docs/evidence checkpoint without rebuilding. Boundary: iPhone Safari remains untested.
