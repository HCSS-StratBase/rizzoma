...
Last Updated: 2026-02-02 (refreshed after perf=full detection update + perf harness run; branch snapshot current)
...
- Perf: `perf-harness.mjs` 200-blip run PASS (TTF 2173.8ms, FCP 260ms, rendered 101/200); 1000-blip runs still FAIL — latest 2026-02-02 run (1000 blips) reported TTF ~65–67s, memory 117MB, labels rendered 1000/1000 in both landing-labels and expanded-root (`snapshots/perf/metrics-1770041471455-*.json`). Windowed 200-label time logged (~5.9s landing, ~66s expanded). `perfLimit` raises `/api/blips` limits in perf mode; `x-rizzoma-perf=1` skips blip history writes during perf seeding and `perf=full` now skips unread/sidebar fetches. Budgets enforced via `scripts/perf-budget.mjs`; still need to lower large-wave TTF/memory and resolve missing root label (1000/1001).
