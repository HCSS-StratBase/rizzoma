...
Current State (`master` target via `codex/production-service-hardening` @ 2026-07-12; public production still `fe6988fb`, PR #60)
...
- A live security preflight proved CouchDB `5984` and unauthenticated Redis `6379` were externally reachable because Docker published them on all interfaces inside the Robot firewall's `5432-6543` allow range. A persistent public-interface `DOCKER-USER` drop now closes both; external retest is closed and public app health remains HTTP 200. See `screenshots/260712-1203-database-exposure-closure/`.
