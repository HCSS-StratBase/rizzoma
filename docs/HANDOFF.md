...
- Tests last run (2026-02-02): Playwright `node test-blb-snapshots.mjs` pass; BLB snapshots refreshed under `snapshots/blb/1770004392410-*` (inline expansion driven via `blip-thread-toggle` dispatch for harness stability). Inline marker click handler now uses event delegation + capture phase. Historical runs for toolbar/follow-green/health remain listed below; re-run before merges.
