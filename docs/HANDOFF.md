...
Last Updated: 2026-04-24 (fresh visual feature sweep added; production perf baseline retained)
...
- Visual sweep residual from 2026-04-24: the generic mobile topic deep-link route can remain on `Loading...`; mobile topic content is evidenced by the follow-green mobile screenshot in the same sweep folder. Next visual sweep should add a row-level `covered_by` matrix for all screenshot-valid documented rows.
...
- Branch focus (current batch): keep changes small/flagged; land perf/resilience sweeps and adapter/health/backup work in slices.
  - Visual sweep hardening: turn `screenshots/260424-003739-feature-sweep/manifest.json` into a row-level coverage matrix for all 161 screenshot-valid rows, keep `npm run visual:sweep` current, and fix mobile deep-link loading if it remains reproducible.
