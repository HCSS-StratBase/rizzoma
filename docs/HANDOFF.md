...
Last Updated: 2026-04-24 (visual coverage matrix added; production perf baseline retained)
...
- Tests last run (2026-04-24): public-prod visual feature sweep passed via `RIZZOMA_BASE_URL=https://138-201-62-161.nip.io RIZZOMA_SWEEP_STAMP=260424-003739 npm run visual:sweep`, with artifacts in `screenshots/260424-003739-feature-sweep/`: 40 primary screenshots, manifest parsing 196 documented rows / 161 screenshot-valid rows / 69 dynamic candidates. `RIZZOMA_SWEEP_DIR=screenshots/260424-003739-feature-sweep npm run visual:coverage` passed and wrote `coverage.md`/`coverage.json` with all 161 screenshot-valid rows classified: 93 static screenshot-covered, 8 dynamic screenshot-covered, 58 non-screenshot/test-artifact, 2 explicit screenshot gaps, 0 needs-review. Supplemental `test:follow-green` desktop+mobile passed into `screenshots/260424-003739-feature-sweep/follow-green/`. Public-prod full-render perf baseline also passed at 100 seeded blips (`RIZZOMA_PERF_RENDER=full`, artifacts in `screenshots/260424-prod-perf-baseline/`): landing-labels stage 1193.7ms, expanded-root stage 524.5ms, FCP 740ms, memory 33-36MB, labels 100/100. `PERF_SNAPSHOT_DIR=screenshots/260424-prod-perf-baseline PERF_BUDGET_EXPECTED_BLIPS=100 PERF_BUDGET_MIN_RATIO=1 node scripts/perf-budget.mjs` passed on stage-local budgets. Optional absolute-TTF diagnostic still flags expanded-root at 3522.9ms vs 3000ms. `RIZZOMA_BASE_URL=https://138-201-62-161.nip.io RIZZOMA_E2E_BROWSERS=chromium RIZZOMA_SNAPSHOT_DIR=screenshots/260424-prod-toolbar-scoped npm run test:toolbar-inline` passed. `npm run test:health` passed (10 tests); `npm test -- --run src/tests/client.BlipMenu.test.tsx` passed (18 tests).
...
- Visual coverage residual from 2026-04-24: live cursors and typing indicators are the only remaining screenshot gaps and need a genuine two-client dynamic capture. The generic mobile topic deep-link route can remain on `Loading...`; mobile topic content is evidenced by the follow-green mobile screenshot in the same sweep folder.
...
- Branch focus (current batch): keep changes small/flagged; land perf/resilience sweeps and adapter/health/backup work in slices.
  - Visual sweep hardening: keep `npm run visual:sweep` plus `npm run visual:coverage` current, add a genuine two-client screenshot for live cursor/typing indicators, and fix mobile deep-link loading if it remains reproducible.
