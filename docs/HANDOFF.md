...
Last Updated: 2026-07-12 (integration candidate `codex/offline-auth-isolation`; PR #65 source base `5a376119`; public runtime code remains `fe6988fb`). Production replay is disabled behind an empty allowlist; REST/Yjs state is owner-partitioned; auth transitions rebootstrap across tabs and rotate Socket.IO with empty packet buffers; the shell exposes Sign in/identity/Logout and becomes read-only offline; service-worker v2 makes `/api`, `/socket.io`, and `/uploads` network-only and purges legacy dynamic caches. Measured gates: targeted **60/60**, full **343 passed / 3 skipped**, typecheck, **3,306-module** build, and **24** visually inspected viewport PNGs with **0** unexpected console errors. This is not deployment acceptance: the combined head must preserve PR #66's socket session/access/revocation/authoritative-user checks and network-only transport policy together with this candidate's acknowledgement/owner/transport-reset checks and v2 cache purge. See the [offline/auth isolation worklog](worklog-260712-offline-auth-isolation.md).
...
Current State (`codex/offline-auth-isolation` based on PR #65 source `5a376119` @ 2026-07-12; public production still `fe6988fb`, PR #60)
- The offline/auth candidate keeps production mutation replay disabled and the UI read-only offline. Queue records, Yjs documents, pending acknowledgements, and unresolved in-memory snapshots are scoped to the authenticated owner; A→logout→B and cross-tab cookie-switch regressions are covered. One accessible shell auth surface renders per viewport. Local code/build/24-PNG gates are green; combined two-real-user browser acceptance remains open.
- Integration dependency: preserve PR #66's authenticated Socket.IO session, per-blip access, revocation, awareness validation, logout disconnect, and authoritative `user.id` sync response together with this branch's owner comparison and update acknowledgement callback. Do not deploy either half alone.
